...
This project includes a small local Express API for MongoDB persistence.

1. Copy `.env.example` to `.env`.
2. Adjust `MONGODB_URI` if your MongoDB instance is not local.
3. Start Angular and the API together:
...
Angular calls `/api` through `proxy.conf.json`, which forwards requests to `http://127.0.0.1:3000`.
